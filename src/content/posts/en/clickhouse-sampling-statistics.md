...
When dealing with PB-scale datasets, how can you get an approximate result quickly without affecting the statistical conclusion? ClickHouse provides an efficient **sampling query technique** that lets you get results with "**95% accuracy using just 1% of the data**."
...
3. Sampling is **deterministic**, so the result does not change for the same query conditions.
...
* The query randomly selects about `k` proportion of data granules for processing.
...
* `N` is the target number of rows to process (approximately).
